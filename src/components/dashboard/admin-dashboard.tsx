...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel('table-updates')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'restaurant_tables' }, payload => {
                // Just refetch for simplicity or splice updates
    ...
    if (!isLoading && sections.length === 0) {
        return <div className="p-8 text-center">No restaurant sections found. Please add sections in Settings.</div>
    }
            ...
            <div className="space-y-8">
                ...
                <div className="space-y-8">
                    {sections.map(section => (
                        <div key={section.id}>
                            <h2 className="text-2xl font-bold font-headline mb-4 pb-2 border-b">{section.name}</h2>
