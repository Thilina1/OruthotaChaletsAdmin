...
const PAGE_SIZE = 10;
function usePageSlice<T>(rows: T[], pageSize = PAGE_SIZE) { const [page, setPage] = useState(1); useEffect(() => setPage(1), [rows.length, pageSize]); const totalPages = Math.max(1, Math.ceil(rows.length / pageSize)); const currentPage = Math.min(page, totalPages); return { pageRows: rows.slice((currentPage - 1) * pageSize, currentPage * pageSize), currentPage, totalPages, setPage, pageSize }; }
type UsageRecord = {
...
export default function KitchenStockUsageReportPage() {
    ...
    const page = usePageSlice(visibleRows);
