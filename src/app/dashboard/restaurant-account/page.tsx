...
export default function RestaurantAccountPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const params = new URLSearchParams(); if (dateFrom) params.set('from', dateFrom); if (dateTo) params.set('to', dateTo); const response = await fetch(`/api/admin/restaurant-account${params.toString() ? `?${params.toString()}` : ''}`, { cache: 'no-store' }); const result = await response.json(); if (!response.ok || result.error) throw new Error(result.error || 'Failed to load restaurant account.'); setData(result); setCardAccountId(result.settings?.card_account_id || ''); }
  ...
  return <div className="space-y-6">
    ...
    <Card><CardContent className="flex flex-wrap items-end gap-3 p-4"><div className="space-y-1"><Label className="text-xs">From</Label><Input type="date" className="w-44" value={dateFrom} max={dateTo || undefined} onChange={event => setDateFrom(event.target.value)} /></div><div className="space-y-1"><Label className="text-xs">To</Label><Input type="date" className="w-44" value={dateTo} min={dateFrom || undefined} onChange={event => setDateTo(event.target.value)} /></div><Button type="button" variant="outline" disabled={!dateFrom && !dateTo} onClick={() => { setDateFrom(''); setDateTo(''); }}>Clear Dates</Button></CardContent></Card>
