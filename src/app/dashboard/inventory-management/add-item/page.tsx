...
export default function RegisterItemPage() {
    ...
    const fetchMetadata = async () => {
        try {
            ...
            toast({ variant: "destructive", title: "Unable to load item data", description: "Please refresh the page to try again." });
        } finally {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok || data.error) throw new Error(data.error || "Failed to register item.");
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">Browse registered items or create a new item for your inventory.</p>
                </div>
                <div className="flex flex-wrap gap-2">
                    <Button variant="outline" asChild>
                        <Link href="/dashboard/inventory-management/item-masters"><Tag className="mr-2 h-4 w-4" /> Inventory Item Masters</Link>
                    </Button>
                <Dialog open={isCreateOpen} onOpenChange={open => { if (!isSubmitting) setIsCreateOpen(open); }}>
                    <DialogTrigger asChild>
                        <Button disabled={isLoadingMetadata}>
                    ...
                    <DialogContent className="max-w-4xl max-h-[90dvh] overflow-y-auto">
                        <DialogHeader>
                            ...
                            <DialogDescription>Add a new item to your inventory catalog.</DialogDescription>
                        </DialogHeader>
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    <CardContent className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-2 items-start gap-4">
                                            ...
                                            <div className="grid min-w-0 content-start gap-2">
                                                ...
                                                >
                                                    <SelectTrigger id="status" className="min-w-0 bg-white">
                                        ...
                                        <div className="grid gap-2">
                                            ...
                                            <CreatableCombobox
                                                options={Array.from(new Set<string>(items.map(item => String(item.brand || '').trim()).filter(Boolean))).sort()}
                                                placeholder="Select or type a brand..."
                                                value={formData.brand}
                                                onValueChange={brand => setFormData(current => ({ ...current, brand }))}
                                                className="bg-white"
                                            />
                                        </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <Button type="button" variant="outline" className="w-full" disabled={isSubmitting} onClick={() => setIsCreateOpen(false)}>
                                    Cancel
                                </Button>
                ...
                <CardHeader>
                    <div className="flex items-center justify-between gap-4 flex-wrap">
                        <div>
                            ...
                            <CardDescription>{items.length} registered items. Click the pencil to rename an item.</CardDescription>
                        </div>
                                    ...
                                    paginatedRegisteredItems.map(item => (
                                        <TableRow key={item.id}>
                                            ...
                                            <TableCell className="text-right">
                                                {editingItemId !== item.id && (
                                                    <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => startEditName(item)} aria-label={`Rename ${item.name}`}>
